...
| --- | --- | --- |
| Direct database read | The web pin calls only the subject-scoped `happy-wakey-lib-core` read capability; infra requires a database-enforced read-only role. | Merged API/web `main` pins; native tests and strict linting validated against the exact private Shared Auth revision. |
| Stateless HTTPS | The web pin uses bounded no-redirect HTTPS and the API re-introspects the bearer with the official typed Shared Auth client. | Merged API/web `main` pins and green hosted repository checks. |
| Stateful TLS | The pins implement asymmetric bounded frames, connection/request limits, TLS verification, reconnect-on-read-failure, and reauthentication on every frame. | Merged API/web `main` pins and bounded transport tests. |
| Async JetStream/outbox | Authenticated HTTPS registers the outbox; the credential-free signal enters a pre-provisioned durable stream; the API commits and durably publishes the response before acknowledging the request. | Merged API/web `main` pins and pre-provisioned durable topology; Core NATS is explicitly forbidden. |

This integration pin uses the merged API revision
`e2cdae8a0f9eb3653e92b9845e1fefebbd53fe43` and merged web revision
`87d69a6cad7f246ea540131cf77b6dc5349b99cb`. Public Actions runners validate
formatting and repository safety without receiving cross-organization source
credentials; native tests and strict linting are run in an authorized context
against the exact pinned private Shared Auth client. Do not claim deployment
readiness without exact image digests and live environment evidence.
